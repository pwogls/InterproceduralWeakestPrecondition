[
    ...
    },
    {
        ...
        "allOf": [
            {
                "anyOf": [
                    {"XXX TODO XXX": "isinstance(y, np.ndarray)"},
    ...
    },
    {
        ...
        "anyOf": [
            {"XXX TODO XXX": "type(clf) in [list, tuple, set, frozenset]"},
            {
                "allOf": [
                    {"XXX TODO XXX": "hasattr(clf, 'get_params')"},
    ...
    },
    {
        ...
        "anyOf": [
            {"XXX TODO XXX": "name not in clf.get_params(False)"},
            {"XXX TODO XXX": "new_object_params[name] is params_set[name]"},
        ],
    },
    {
        "description": "From /ensemble/_voting.py:_BaseVoting:fit, Exception: raise ValueError(     'Number of `estimators` and weights must be equal; got %d weights, %d estimators'      % (len(self.weights), len(self.estimators))) ",
        "anyOf": [
            {"type": "object", "properties": {"weights": {"enum": [None]}}},
            {"XXX TODO XXX": "len(self.weights) == len(self.estimators)"},
        ],
    },
    {
        "description": "From /ensemble/_voting.py:VotingClassifier:fit, Exception: raise NotImplementedError(     'Multilabel and multi-output classification is not supported.') ",
        "anyOf": [
            {"XXX TODO XXX": "isinstance(y, np.ndarray)"},
            {"XXX TODO XXX": "len(y.shape) <= 1"},
            {"XXX TODO XXX": "y.shape[1] <= 1"},
    ...
    {
        "description": "From /ensemble/_voting.py:VotingClassifier:fit, Exception: raise ValueError(\"Voting must be 'soft' or 'hard'; got (voting=%r)\" % self.     voting) ",
        "type": "object",
