[
        ...
        "allOf": [
            {
                "anyOf": [
                    {
                        "type": "object",
                        "properties": {"algorithm": {"enum": ["SAMME", "SAMME.R"]}},
                    },
                    {
                        "allOf": [
                            {
                ...
                "anyOf": [
                    {
                        "type": "object",
                        "properties": {
                            "algorithm": {"not": {"enum": ["SAMME", "SAMME.R"]}}
                        },
                    },
                    {
                        "allOf": [
        ...
        "allOf": [
            ...
            },
            {
                "anyOf": [
                    ...
                    {"XXX TODO XXX": "DecisionTreeClassifier(max_depth=1) is not None"},
                ]
            },
        ],
    },
    {
        "description": 'From /ensemble/_weight_boosting.py:AdaBoostClassifier:_validate_estimator, Exception: raise TypeError(     """AdaBoostClassifier with algorithm=\'SAMME.R\' requires that the weak learner supports the calculation of class probabilities with a predict_proba method. Please change the base estimator or set algorithm=\'SAMME\' instead."""     ) ',
        "anyOf": [
            {
                "type": "object",
                "properties": {"algorithm": {"not": {"enum": ["SAMME.R"]}}},
            },
            {"XXX TODO XXX": "hasattr(self.base_estimator_, 'predict_proba')"},
        ],
    },
    {
        "description": 'From /ensemble/_weight_boosting.py:AdaBoostClassifier:_validate_estimator, Exception: raise ValueError("%s doesn\'t support sample_weight." % self.base_estimator_     .__class__.__name__) ',
        "XXX TODO XXX": "has_fit_parameter(self.base_estimator_, 'sample_weight')",
    },
    {
    ...
    },
    {
        ...
        "anyOf": [
            {"XXX TODO XXX": "iboost not in range(self.n_estimators)"},
            {"type": "object", "properties": {"algorithm": {"enum": ["SAMME.R"]}}},
            {
                "XXX TODO XXX": "type(self.base_estimator_) in [list, tuple, set, frozenset]"
            },
            {
    ...
    {
        "description": "From /ensemble/_weight_boosting.py:AdaBoostClassifier:_boost_discrete, Exception: raise ValueError(     'BaseClassifier in AdaBoostClassifier ensemble is worse than random, ensemble can not be fit.'     ) ",
        "anyOf": [
            {"XXX TODO XXX": "iboost not in range(self.n_estimators)"},
            {"type": "object", "properties": {"algorithm": {"enum": ["SAMME.R"]}}},
            {
                "allOf": [
                    {
                        "anyOf": [
                            {
                                "type": "object",
                                "properties": {"iboost": {"not": {"enum": [0]}}},
                            },
                            {
                                "XXX TODO XXX": "np.mean(np.average(self._make_estimator(random_state=check_random_state(self.random_state)).predict(X) != y, weights=_check_sample_weight(sample_weight, X, np.float64) / _check_sample_weight(sample_weight, X, np.float64).sum(), 0)) <= 0"
                            },
                            {
                                "XXX TODO XXX": "np.mean(np.average(self._make_estimator(random_state=check_random_state(self.random_state)).predict(X) != y, weights=_check_sample_weight(sample_weight, X, np.float64) / _check_sample_weight(sample_weight, X, np.float64).sum(), 0)) < 1.0 - 1.0 / len(getattr(self._make_estimator(random_state=check_random_state(self.random_state)), 'classes_', None))"
                            },
                        ]
                    },
                    {
                        "anyOf": [
                            {"type": "object", "properties": {"iboost": {"enum": [0]}}},
                            {
                                "XXX TODO XXX": "np.mean(np.average(self._make_estimator(random_state=check_random_state(self.random_state)).predict(X) != y, weights=_check_sample_weight(sample_weight, X, np.float64) / _check_sample_weight(sample_weight, X, np.float64).sum(), 0)) <= 0"
                            },
                            {
                                "XXX TODO XXX": "np.mean(np.average(self._make_estimator(random_state=check_random_state(self.random_state)).predict(X) != y, weights=_check_sample_weight(sample_weight, X, np.float64) / _check_sample_weight(sample_weight, X, np.float64).sum(), 0)) < 1.0 - 1.0 / self.n_classes_"
                            },
                        ]
                    },
                ]
    ...
    {
        "description": "From /ensemble/_weight_boosting.py:AdaBoostClassifier:fit, Exception: raise ValueError('algorithm %s is not supported' % self.algorithm) ",
        "type": "object",
        "properties": {"algorithm": {"enum": ["SAMME", "SAMME.R"]}},
    },
]
